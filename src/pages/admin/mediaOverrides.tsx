...
const AdminMediaOverrides: FunctionComponent = () => {
  ...
  useEffect(() => {
    (async () => {
      ...
      const response = await fetch(prefixUrl(`/api/v1/db/mediaOverrides`));
      const data: MediaOverrideList[] = await response.json();
      setRecords(data);
    })();
  }, [navigate]);
  ...
  const handleDelete = async (id: number) => {
    ...
    await fetch(prefixUrl(`/api/v1/db/mediaOverrides/${id}`), { method: "DELETE" });
    setRecords(records.filter((record) => record.id !== id));
  };

  // Group records by date
  const groupedRecords = useMemo(() => {
    const groups: Record<string, MediaOverrideList[]> = {};
      ...
      <div className={adminCommon.container}>
        ...
        <section className={adminCommon.section}>
          ...
          <div className={adminCommon.details}>
            <Link to="/admin/mediaOverridesUpsert" className={adminCommon.createButton}>
              + Create Override
            </Link>

            {records.length === 0 ? (
              <p className={adminCommon.emptyState}>No media overrides found.</p>
            ) : (
                    ...
                    <ul className={adminCommon.dateGroupRecords}>
                      {dateRecords.map((record) => (
                        <li key={record.id} className={adminCommon.recordItem}>
                          <Link
                            to={`/admin/mediaOverridesUpsert?id=${record.id}`}
...
export default AdminMediaOverrides;
