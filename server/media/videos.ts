...
import { fetchVideoDataService } from "server/services/io-api";
import { fetchDatetimeOverrides } from "server/services/wiki-api";
...
export default async function getVideoData(
  ...
  const [results, overrides] = await Promise.all([
    ...
    fetchVideoDataService(collection, previousDate, nextDate),
